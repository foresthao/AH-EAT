...
from model.adversarial import PGDAdversary
'''
适用数据集NF-BoT-IoT、NF-ToN-IoT、NF-UNSW-NB15-v2
'''
print('开始进行数据的预处理')
print('1.读入数据')
...
# data = data.groupby(by='Attack').sample(frac=0.05, random_state=13)
...
encoder = ce.TargetEncoder(cols=['TCP_FLAGS','L7_PROTO','PROTOCOL'])#目标编码器类，用于对分类特征进行编码
...
adv_G = pgd_adversary.generate(G=G_test, target='alternate')

print('4.保存')
# 使用 pickle 保存图
save_path = os.path.join(dataset_path, "graph/","saved_graph_cic_ids18_5%.pkl")
save_path_test = os.path.join(dataset_path, "graph/","saved_graph_cic_ids18_5%_test.pkl")
