...
def train(model, G, device, args):
    ...
    criterion = AdaptiveHierarchicalLoss3(
    ...
    for epoch in range(args.num_epochs):
        ...
        # 数值稳定性保护
        coarse_pred = torch.clamp(coarse_pred, min=-50, max=50)
        fine_pred = torch.clamp(fine_pred, min=-50, max=50)
...
def evaluate(model, G_test, device):
    ...
    with torch.no_grad():
        ...
        if mask.sum() > 0:
            fine_labels = G_test.edata['Attack'][test_mask][mask]
            # fine_acc = (fine_pred[test_mask][mask].argmax(1) == fine_labels).float().mean().item()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='RoNGAT_hierarchical model')
